...
export function generateDataset(
  ...
  const buildWitnessGroups = (members: typeof shuffled) => {
  ...
  const addGroup = (
    ...
    if (kindValue === 'ring') {
      const witnessGroups = buildWitnessGroups(members);

      for (const [groupIndex, group] of witnessGroups.entries()) {
        const witnessKey = `${kindValue}:account:${seed}:${ordinal}:${groupIndex}`;
        for (const member of group) {
          ...
          if (rng() < 0.55) {
            ...
            if (device)
              device.rawValue = `${kindValue}:device:${seed}:${ordinal}:${groupIndex}`;
          }
        }
      }
    }

    if (kindValue === 'legitimate_dense') {
      for (const [memberIndex, member] of members.entries()) {
        ...
        if (
          account &&
          ((memberIndex % 2 === 0 && ordinal % 2 === 0) ||
            (memberIndex % 3 === 0 && ordinal % 3 === 0))
        ) {
          account.rawValue = sharedAccount;
        }
        if (device && memberIndex % 2 === 0) {
